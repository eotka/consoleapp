...
public class devsSystem
{
	...
	/*static void Removedev() // Удаление книги
	{
		Console.Write("Введите название книги или: ");
		string data = Console.ReadLine();
		string[] remdev = SearchingDev(data);
		if (!(remdev.Length == 1))
		{
			if(remdev.Length == 0)
			{
		...
		var tempFile = globalVar.path.GetTempFileName();
		var linesToKeep = File.ReadLines(globalVar.path).Where(l => l != remdev[0]);
		File.WriteAllLines(tempFile, linesToKeep);
		File.Delete(globalVar.path);
		File.Move(tempFile, globalVar.path);
